...
You are a **Target of Intent** — Steve forked `MemPalace/mempalace` as a deliberate
...
```bash
gh issue list -R MemPalace/mempalace --limit 15 --sort updated --json number,title,updatedAt,labels 2>/dev/null
```

### 4. Scan upstream PRs (active and recently merged)
```bash
gh pr list -R MemPalace/mempalace --limit 10 --state all --sort updated --json number,title,state,updatedAt 2>/dev/null
```
...
of `MemPalace/mempalace`. The doctrine has two clauses:
