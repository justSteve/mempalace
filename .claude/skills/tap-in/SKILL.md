...
```bash
head -1 /root/projects/mempalace/DaysActivity.md 2>/dev/null
```
...
```bash
head -80 /root/projects/mempalace/DaysActivity.md
```
...
```bash
cat /root/projects/mempalace/CurrentStatus.md
```
...
```bash
tail -30 /root/projects/mempalace/.beads/issues.jsonl | jq -r 'select(.status == "open") | [.id, .type, .title] | @tsv' | column -t
```
...
CM_PORT=$(grep -E '^CM_PORT=' /root/projects/mempalace/factory/factory.env 2>/dev/null | cut -d'"' -f2)
...
jq -r 'select(.status == "open") | [.id, .title, .metadata.team // "none", .metadata.priority // "none"] | @tsv' \
  /root/projects/mempalace/.beads/issues.jsonl | column -t
```
...
jq -r 'select(.status == "open" and .metadata.team) | .metadata.team' \
  /root/projects/mempalace/.beads/issues.jsonl | sort | uniq -c
```
...
**Write to**: `/root/projects/mempalace/session-briefing.md`
